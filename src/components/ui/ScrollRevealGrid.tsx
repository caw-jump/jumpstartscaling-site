...
function CountUpMetric({ label, value, delay }: { label: string, value: string, delay: number }) {
    ...
    return (
        <motion.div
            ...
            className="glass-card p-12 md:p-16 text-center h-full flex flex-col justify-center items-center hover:border-[var(--accent)] transition-colors duration-300"
        >
            <h3 className="text-5xl md:text-6xl font-bold gradient-text mb-6 leading-none">{displayValue}</h3>
            <p className="text-xl md:text-2xl text-[var(--text-secondary)] font-medium">{label}</p>
        </motion.div>
    );
}
